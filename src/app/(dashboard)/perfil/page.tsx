...
export default function PerfilPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/membros/${user.memberId}`)
      .then((r) => r.json())
      .then((data) => {
        form.reset({
  ...
  async function onSubmit(values: PerfilData) {
    ...
    setSaving(true)
    try {
      // Fetch current member data to merge (protected fields must be sent as-is)
      const res = await fetch(`/api/membros/${user.memberId}`)
      ...
      const saveRes = await fetch(`/api/membros/${user.memberId}`, {
        method: "PUT",
