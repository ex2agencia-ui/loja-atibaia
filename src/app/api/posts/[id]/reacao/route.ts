...
// Toggle: se já existe remove, se não existe cria
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.reacao.findUnique({
    where: { memberId_postId_emoji: { memberId: user.memberId, postId, emoji } },
  })
  ...
  await prisma.reacao.create({ data: { emoji, memberId: user.memberId, postId } })
